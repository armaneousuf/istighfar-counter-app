...
const insightAvgTaps = document.getElementById("insightAvgTaps");
...
function renderPlaygroundChips() {
  ...
  pgTargetChips.forEach((chip) => {
    ...
    const isActive =
      chipTarget === "custom" ? !isPreset : Number(chipTarget) === target;
...
function updatePlaygroundDisplay() {
  ...
  if (pgTargetLabel)
    pgTargetLabel.textContent = state.playgroundTarget.toLocaleString();
  if (pgRoundsToday)
    pgRoundsToday.textContent = state.playgroundRoundsToday.toLocaleString();
  ...
  const percent =
    state.target > 0
      ? Math.min(
          100,
          Math.floor(
            ((isAnonymous ? anonymousCount : state.count) / state.target) * 100,
          ),
        )
      : 0;
...
function renderInsightSummary() {
  ...
  const previousWeeklyTotal = getRecentDays(7, 7).reduce(
...
function enableFocusMode() {
  if (focusCount)
    focusCount.textContent = (
      isAnonymous ? anonymousCount : state.count
    ).toLocaleString();
  ...
  chip.addEventListener("click", () => {
    ...
    if (raw === "custom") {
      const input = window.prompt(
        "Set a custom target for this round:",
        state.playgroundTarget,
      );
...
const APP_LAST_UPDATED = "2026-09-21";
